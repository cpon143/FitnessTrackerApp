...
class YogaSessionDatabaseHelper(context: Context) :
    SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {

    companion object {
        const val DATABASE_NAME = "yoga_sessions.db"
        const val DATABASE_VERSION = 1
        const val TABLE_NAME = "yoga_sessions"

        const val COL_ID = "id"
        const val COL_USER_ID = "user_id" // New column
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val createTable = """
            CREATE TABLE $TABLE_NAME (
                ...
                $COL_USER_ID INTEGER NOT NULL,  -- Associate session with user
    ...
    // Retrieve sessions only for a specific user
